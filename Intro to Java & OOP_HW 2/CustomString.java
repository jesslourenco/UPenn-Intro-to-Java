 ...
 * For example, the CustomString class has a â€œreverseâ€� method which returns a new string version of the current string where the 
 ...
 * in the given arg.  For CustomString â€œabc, XYZ; 123.â€�, calling reverse("bcdxyz@3210.") will return "aBC, xyz; 123.".  
 * 
 * The CustomString class also has a â€œremoveâ€� method which returns a new string version of the current string where the alphabetical 
...
public class CustomString {
	...
	public String remove(String arg) {
		if (myString == null || !isSet || myString == "") {
			return "";
		}
		...
		for (int j = 0; j < myString.length(); j++) {
			...
			for (int i = 0; i < arg.length(); i++) {
				if (Character.isLetter(arg.charAt(i))) {
					if (arg.charAt(i) == myString.charAt(j)) {
						found = true;
						break;
					}
				}
	...
	public String reverse(String arg) {
		...
		for (int j = 0; j < myString.length(); j++) {
			...
			for (int i = 0; i < arg.length(); i++) {
				if (Character.isLetter(arg.charAt(i))){
					if (Character.toLowerCase(arg.charAt(i)) == Character.toLowerCase(myString.charAt(j))) {
						found = true;
						if (Character.isUpperCase(myString.charAt(j))) {
							newString += Character.toLowerCase(myString.charAt(j));
							continue;
						}
						newString += Character.toUpperCase(myString.charAt(j)); 
					}					
				}
			}
			if(!found) 
	...
	public String filterLetters(char n, boolean more) {
		if (!Character.isLetter(n) || !isSet) return "";
		...
		for (int i = 0; i < myString.length(); i++) {
			...
			if (more && Character.toLowerCase(letter) >= n) continue;
			if (!more && Character.toLowerCase(letter) <= n) continue;
			newString += letter;
		}
		return newString;
